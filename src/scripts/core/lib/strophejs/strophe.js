...
Strophe.Bosh.prototype = {
    ...
    _onRequestStateChange: function (func, req)
    {
        ...
        if (req.xhr.readyState == 4) {
            ...
            // 检测是否是度假报警消息拦截
            // 目前发现只有这个场景下才会先返回后发送的response
            // 正常情况下reqIs1=false，这里会等于true
            // 这时候消息其实是能发出去的，同时阻止重发，消息计数rid--
            // 以此恢复被破坏的xmpp消息栈顺序 - 暂时没有找到更好的修改方法
            // baotong.wang@2017-04-25
            var stopRestart = reqIs1 && 
                req.xhr.responseText.indexOf('dujia-ops') > 0 && 
                req.xhr.responseText.indexOf('web-overbalance') > 0;
            
            if(stopRestart) {
                this.rid--;
                this._removeRequest(this._requests[0]);
            }
            ...
            if (reqStatus == 200) {
                ...
                if (reqIs1 ||
                    ...
                    stopRestart || this._restartRequest(0);
                }
                // call handler
                Strophe.debug("request id " +
                              req.id + "." +
    ...
    _processRequest: function (i)
    {
        ...
        if (req.xhr.readyState === 0) {
            ...
            try {
                ...
                if(req.xhr && req.xhr.setRequestHeader){
